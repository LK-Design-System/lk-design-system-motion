 ...
 * SlideStage 지오메트리, 엔진(core/), 렌더러를 건드린 뒤에는 반드시 돌린다.
 * ResizeObserver·rAF·Date.now()·Math.random()에 기대는 코드가 섞이면 여기서
 * 잡힌다.
 *
...
const render = (name) => {
	execFileSync(
		process.execPath,
		['scripts/render.mjs', composition, `${dir}/${name}.png`, `--frame=${frame}`],
		{stdio: 'pipe'},
	);
	return createHash('sha256')
		.update(readFileSync(`${dir}/${name}.png`))
		.digest('hex');
};
